...
if [ ! -f $ISDF ]; then
    for f in .emacs.d/* .gnus.el .config/dunst/dunstrc .config/terminator/config .gitconfig .i3/config .i3/autostart .i3status.conf .irssi/config .zfunctions/* .zshrc ; do
	cp $f ~/$(dirname $f)
    done
    touch $ISDF
fi
